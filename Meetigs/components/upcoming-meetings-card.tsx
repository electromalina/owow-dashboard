...
function downloadCalendarInvite(meeting: Meeting) {
  ...
  const end = new Date(start.getTime() + parseDurationToMinutes(meeting.duration) * 60_000)
  ...
  const description =
    meeting.type === "video"
      ...
      : "In-person meeting"
  const location = meeting.type === "video" ? meeting.videoUrl ?? "Online" : meeting.location ?? "TBD"
...
export function UpcomingMeetingsCard() {
  return (
    <div className="w-[682px] h-[328px] rounded-xl border border-[#2a2a2a] bg-[#141414] p-5 flex flex-col">
      {/* Header */}
        ...
        {meetings.map((meeting) => (
          ...
          >
            ...
            <div className="flex items-center gap-4">
              {/* Date - Bigger */}
              <div className="flex flex-col items-center justify-center rounded-lg bg-[#252525] px-3 py-2 min-w-[60px]">
                <span className="text-lg font-semibold text-white">{meeting.date.split(" ")[1]}</span>
                <span className="text-[10px] tracking-wide text-[#888]">{meeting.date.split(" ")[0]}</span>
              </div>
              ...
              {/* Add-to-calendar button for next meeting */}
              {meeting.isNext && (
                <button
